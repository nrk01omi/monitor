// Infrastructure topology view — Cytoscape graph + Chart.js latency history.
// Talks to /api/monitor/{status,topology,history}.
...
(function () {
  ...
  function initCy() {
    cy = cytoscape({
      ...
      style: [
        ...
        { selector: 'node[group = "llm-proxy"]', style: { 'width': 70, 'height': 70 } },
        {
          selector: 'edge',
          style: {
            'width': 1.5,
  ...
  // Hierarchical layout — hands rows of related groups so the topology
  // reads top-down: clients → proxy → backends → peripherals.
  function computeLayout(nodes) {
    ...
    const positions = {};
    const containerWidth = document.getElementById('cy').clientWidth || 1000;

    let xCursor = 80;
    for (const g of topGroups) {
      const items = nodes.filter(n => n.group === g);
      items.forEach((n, i) => { positions[n.id] = { x: xCursor + i * 130, y: groups[g].y }; });
    ...
    for (const g of bottomGroups) {
      ...
      items.forEach((n, i) => { positions[n.id] = { x: startX + i * 140, y: groups[g].y }; });
    }
    return positions;
  }
  ...
  async function fetchAndRender() {
    try {
      ...
      const cyEdges = edges
        ...
        .map((e, i) => ({
          group: 'edges',
          data: { id: `e-${i}`, source: String(e.from), target: String(e.to), label: e.label || '' }
        }));
  ...
  function renderStats(nodes) {
    ...
    document.getElementById('stats').innerHTML = `
      <span class="pill"><span class="dot up"></span>UP ${counts.up}</span>
      <span class="pill"><span class="dot degraded"></span>不安定 ${counts.degraded}</span>
      <span class="pill"><span class="dot down"></span>DOWN ${counts.down}</span>
    `;
  }
  ...
  function renderSide() {
    ...
    if (node.type === 'ollama_backend' && node.detail?.models) {
      const tags = node.detail.models.map(m => `<span class="model-tag">${escapeHtml(m)}</span>`).join('');
      detailHtml = `<h2 style="margin-top:18px">ロード済みモデル</h2><div class="models-list">${tags || '<span class="empty">なし</span>'}</div>`;
    }
    ...
    if (node.type === 'http' && node.detail?.http_status) {
      detailHtml += `<div class="field"><span class="field-key">HTTP status</span><span class="field-val">${node.detail.http_status}</span></div>`;
    }
    if (node.type === 'tcp' && node.detail?.host) {
      detailHtml += `<div class="field"><span class="field-key">Endpoint</span><span class="field-val">${escapeHtml(node.detail.host)}:${node.detail.port}</span></div>`;
    }
    if (node.detail?.url && node.type !== 'http') {
  ...
  async function loadHistory(targetId) {
    try {
      const res = await fetch(`${API}/history/${encodeURIComponent(targetId)}?hours=1`);
  ...
  function drawChart(points) {
    ...
    chart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          x: { ticks: { color: '#8b949e', maxTicksLimit: 6, font: { size: 10 } }, grid: { color: 'rgba(255,255,255,0.05)' } },
          y: { ticks: { color: '#8b949e', font: { size: 10 } }, grid: { color: 'rgba(255,255,255,0.05)' }, beginAtZero: true },
        }
      }
    });
  }
